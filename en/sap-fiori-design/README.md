...
```text
$sap-fiori-design Review the existing webapp/ project against the Fiori guidelines, accessibility, and deprecated API use; report findings with file and line. Do not change files.
```
...
- **Incremental workspace:** the contract is created once; when moving from the prototype to code, run `--output all` in the same folder and the contract the designer completed is kept. `--force` refreshes only the scaffold trees.
...
- **Validator:** `error` and `warning` close the gate, `info` does not. The gate does not stop at form: template text left in the contract, a text key or action without a counterpart in the UI, empty accessibility evidence and an unverified `$search`/FLP inbound are findings too. `--allow-warnings` is only for work in progress; `--review` inspects an existing project that has no contract.
- **Prototype states:** `prototype/index.html?state=loading|empty|no-results|error|no-auth` reproduces every designed state; PNG captures are taken from these addresses.
...
The tests cover scaffold inputs and incremental runs, the delivery gate refusing template text and opening for a completed contract, the contract ↔ UI mapping, review mode, ABAP package reading (abapGit/ADT service binding formats, multi-entity behavior definitions, element lists with inline annotations), ZIP path safety, hash integrity, schema enforcement, and the version check.
...
- [Behavior checks](references/behavior-checks.md): FD01–FD32 are maintenance scenarios, not a claim that every scenario passes in every environment. Not read at runtime.
...
The current version is 1.2.0 and lives directly here, not in `latest/` or a version-numbered subfolder. Previously published releases are kept in `archived/` as unmodified ZIP copies following the repository's [versioning instructions](../CONTRIBUTING.md). Do not extract those ZIPs inside a skill discovery directory.
